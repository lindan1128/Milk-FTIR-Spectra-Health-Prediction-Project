...
def double_cv(X, y, units_range, cv1=5, cv2=5, n_repeats_cv2=5):
    """
    Perform double cross-validation for LSTM model.
    
    Args:
        X (np.ndarray): Feature matrix
        y (np.ndarray): Target labels
        units_range (list): Range of units values to test
    ...
    try:
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X)):
                ...
                for units in units_range:
                    ...
                    for inner_fold_idx, (train_inner_idx, val_idx) in enumerate(inner_cv.split(X_rest)):
                        ...
                        # Train model
                        model = build_lstm_model(X_train.shape[1:], units)
                        model.fit(
                            X_train, y_train,
                            epochs=50,
                            batch_size=32,
                    ...
                    # Use validation AUC to select best units
                ...
                final_model = build_lstm_model(X_rest.shape[1:], best_units)
                final_model.fit(
                    X_rest, y_rest,
                    epochs=50,
                    batch_size=32,
                ...
                try:
                    importance = calculate_feature_importance(final_model, X_rest)
        ...
        return np.array(test_predictions), np.array(test_true), feature_importance, avg_inner_cv_performance, best_units_list, detailed_cv_metrics
    except Exception as e:
...
def main(input_file, output_dir, repeat=50, cv2=5, cv1=5, n_repeats_cv2=5, run_permutation=True):
    ...
    all_best_units = []  # Store best units values
    ...
    for time_group in time_groups:
        ...
        for feature_type in feature_types:
            ...
            # Select units range based on feature type
            if 'spc' in base_feature_type:
                if '_max20' in feature_type:
                    units_range = [32, 64, 128, 256, 512]
                    print("Using extended units_range for spectral data:", units_range)
                else:
                    units_range = [32, 64, 128, 256]
                    print("Using extended units_range for spectral data:", units_range)
            else:
                units_range = [32, 64, 128]
                print("Using standard units_range:", units_range)
            ...
            for i in range(repeat):
                ...
                pred, true, importance, inner_cv_perf, best_units, detailed_cv_metrics = double_cv(
                    X, y, 
                    units_range=units_range,
                    cv1=cv1, 
                    cv2=cv2,
                ...
                # Record best units values
                for units_info in best_units:
                    all_best_units.append({
                        'time_group': time_group,
                        'feature_type': feature_type,  # Use original feature type name (with _max20 suffix)
                        'repeat': i + 1,
                        'fold': units_info['fold'],
                        'best_units': units_info['best_units'],
                        'best_val_auc': units_info['best_val_auc']
                    })
    ...
    if all_best_units:
        best_units_df = pd.DataFrame(all_best_units)
        best_units_df.to_csv(os.path.join(output_dir, 'best_units.csv'), index=False)
        print(f"\nSaved best units values to best_units.csv")
        
        # Calculate and save summary statistics
        summary_stats = best_units_df.groupby(['time_group', 'feature_type'])['best_units'].agg([
            'mean', 'std', 'min', 'max', 'median'
        ]).round(2)
        summary_stats.to_csv(os.path.join(output_dir, 'best_units_summary.csv'))
        print(f"Saved best units summary statistics to best_units_summary.csv")
