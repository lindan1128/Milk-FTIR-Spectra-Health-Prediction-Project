...
def double_cv(X, y, n_estimators_range, cv1=5, cv2=5, n_repeats_cv2=5):
    """
    Perform double cross-validation for Random Forest model.
    
    Args:
        X (np.ndarray): Feature matrix
        y (np.ndarray): Target labels
        n_estimators_range (list): Range of n_estimators values to test
    ...
    try:
        ...
        return np.array(test_predictions), np.array(test_true), feature_importance, avg_inner_cv_performance, best_n_estimators_list, detailed_cv_metrics
    except Exception as e:
...
def main(input_file, output_dir, repeat=50, cv2=5, cv1=5, n_repeats_cv2=5, run_permutation=True):
    ...
    all_best_n_estimators = []  # Store best n_estimators values
    ...
    for time_group in time_groups:
        ...
        for feature_type in feature_types:
            ...
            # Select n_estimators range based on feature type
            if 'spc' in base_feature_type:
                if '_max20' in feature_type:
                    n_estimators_range = [50, 100, 200, 300, 400, 500]
                    print("Using extended n_estimators_range for spectral data:", n_estimators_range)
                else:
                    n_estimators_range = [50, 100, 200, 300]
                    print("Using extended n_estimators_range for spectral data:", n_estimators_range)
            else:
                n_estimators_range = [50, 100, 200]
                print("Using standard n_estimators_range:", n_estimators_range)
            ...
            for i in range(repeat):
                ...
                pred, true, importance, inner_cv_perf, best_n_estimators, detailed_cv_metrics = double_cv(
                    X, y, 
                    n_estimators_range=n_estimators_range,
                    cv1=cv1, 
                    cv2=cv2,
                ...
                # Record best n_estimators values
                for n_est_info in best_n_estimators:
                    all_best_n_estimators.append({
                        'time_group': time_group,
                        'feature_type': feature_type,  # Use original feature type name (with _max20 suffix)
                        'repeat': i + 1,
                        'fold': n_est_info['fold'],
                        'best_n_estimators': n_est_info['best_n_estimators'],
                        'best_val_auc': n_est_info['best_val_auc']
                    })
    ...
    if all_best_n_estimators:
        best_n_estimators_df = pd.DataFrame(all_best_n_estimators)
        best_n_estimators_df.to_csv(os.path.join(output_dir, 'best_n_estimators.csv'), index=False)
        print(f"\nSaved best n_estimators values to best_n_estimators.csv")
        
        # Calculate and save summary statistics
        summary_stats = best_n_estimators_df.groupby(['time_group', 'feature_type'])['best_n_estimators'].agg([
            'mean', 'std', 'min', 'max', 'median'
        ]).round(2)
        summary_stats.to_csv(os.path.join(output_dir, 'best_n_estimators_summary.csv'))
        print(f"Saved best n_estimators summary statistics to best_n_estimators_summary.csv")
